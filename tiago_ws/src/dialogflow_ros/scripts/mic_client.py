...
class GspeechClient(object):
    ...
    def _listen_print_loop(self, responses):
        ...
        for response in responses:
            ...
            if result.is_final:
                ...
                if transcript.startswith(b' '):
                    transcript = transcript[1:]
                # Exit if needed
                if transcript.lower() == b'exit':
                    self.shutdown()

                # Determine here if it is an event or string etc
                # if (True):  # Insert Conditions here
                    # Can change what type of request it would publish
    ...
    def gspeech_client(self):
        ...
        except google.api_core.exceptions.OutOfRange as error:
            print('Input stream has timed out - can start mic_client again if you wish')
