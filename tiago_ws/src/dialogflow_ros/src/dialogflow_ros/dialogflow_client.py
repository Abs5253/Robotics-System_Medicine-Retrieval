...
# Dialogflow
import dialogflow_v2beta1.types
import dialogflow_v2beta1
import yaml
from dialogflow_v2beta1.types import Context, EventInput, InputAudioConfig, QueryInput, QueryParameters, StreamingDetectIntentRequest, TextInput
from dialogflow_v2beta1.types import OutputAudioConfig

from dialogflow_v2beta1.gapic.enums import AudioEncoding  # , OutputAudioEncoding
from dialogflow_v2beta1.gapic.enums import OutputAudioEncoding
import google.api_core.exceptions
import utils
from utils import converters
from utils import output as utils_output
...
class DialogflowClient(object):
    def __init__(self, language_code='en-US', last_contexts=None):
        ...
        with open(file_dir, 'r') as f:
            try:
                # python3 needs FullLoader self.phrase_hints = load(Loader=yaml.FullLoader, stream=f)
                self.phrase_hints = load(stream=f)
        ...
        rospy.Subscriber('/ongoing_tiago_task', Bool, self.set_ongoing_task)
        ...
        if self.PLAY_AUDIO:
            try:
                ...
                print('Audio Output not working - retry if you wish (quit terminal, insert device, try again - or just continue without output)')
    ...
    def set_ongoing_task(self, ongoing_task):
        self.PLAY_AUDIO = not ongoing_task
    ...
    def _create_audio_output(self):
        ...
        # print('create_audio_output: Audio Stream being created')
    ...
    def _play_stream(self, data):
        ...
        """
        print('DF_Client line 193: Audio should be playing now... check headphone connection & volume')
        # print(data)
        # file = open('Audio_test.wav', 'r')
        # file.write(data)
        # file.close
        try:
            ...
            print('Audio output not working, continue without hearing response; or try closing all terminal tabs and reopen')
    ...
    def detect_intent_text(self, msg):
        ...
        try:
            ...
            rospy.loginfo(utils_output.print_result(response.query_result))
            # Play audio
            if self.PLAY_AUDIO:
    ...
    def detect_intent_stream(self, return_result=False):
        ...
        try:
            ...
            rospy.loginfo(utils_output.print_result(final_result))
            # Play audio
            if self.PLAY_AUDIO:
